...
bool test_qson_deserialize_object_entry_auto() {
	test_run_log("qson_deserialize_object_entry_auto");
	char buffer[] = "{ \"key\\u2764\" : \"string value\"  }";
	qson_deserialize_ctx_t ctx;
	char *key;
	size_t key_size;
	...
	success &= qson_deserialize_ctx_create(&ctx, buffer, array_len(buffer)) == QSON_RESULT_OK;
	success &= qson_deserialize_object_start(ctx) == QSON_RESULT_OK;
	success &= qson_deserialize_object_entry_auto(ctx, &key, &key_size, &value_type) == QSON_RESULT_OK;
	success &= key_size == 7;
	success &= strcmp(key, "key\u2764") == 0;
	free(key);
	success &= qson_deserialize_ctx_destroy(ctx) == QSON_RESULT_OK;
	...
	success &= qson_deserialize_ctx_create(&ctx, buffer, array_len(buffer)) == QSON_RESULT_OK;
	success &= qson_deserialize_object_start(ctx) == QSON_RESULT_OK;
	success &= qson_deserialize_object_entry_auto(ctx, NULL, NULL, &value_type) == QSON_RESULT_OK;
	success &= qson_deserialize_ctx_destroy(ctx) == QSON_RESULT_OK;
...
bool test_qson_deserialize_object_entry_value_skip() {
	test_run_log("qson_deserialize_object_entry_value_skip");
	...
	success &= qson_deserialize_ctx_create(&ctx, buffer, array_len(buffer)) == QSON_RESULT_OK;
	success &= qson_deserialize_object_start(ctx) == QSON_RESULT_OK;
	if (qson_deserialize_ctx_state(ctx) == QSON_DESERIALIZING_STATE_OBJECT) {
		bool has_next = true;
		while (has_next && success) {
			qson_type_t value_type = QSON_TYPE_AUTO;
			success &= qson_deserialize_object_entry(ctx, NULL, NULL, &value_type) == QSON_RESULT_OK;
			success &= qson_deserialize_object_entry_value_skip(ctx, &has_next) == QSON_RESULT_OK;
		}
	}
...
bool test_qson_deserialize_object_skip() {
	test_run_log("qson_deserialize_object_skip");
	char buffer[] = "{ \" \": false, \" \": \"sik\", \" \": 2, \" \": null, \" \": { \"\": 85 }, \" \": {} }w";
	qson_deserialize_ctx_t ctx;

	bool success = 1;
	success &= qson_deserialize_ctx_create(&ctx, buffer, array_len(buffer)) == QSON_RESULT_OK;
	success &= qson_deserialize_object_skip(ctx) == QSON_RESULT_OK;
	success &= buffer[qson_deserialize_ctx_index(ctx)] == 'w';
	success &= qson_deserialize_ctx_destroy(ctx) == QSON_RESULT_OK;
	test_result_log(success);
	return success;
...
bool test_qson_deserialize_object() {
	...
	success &= test_qson_deserialize_object_entry_value_skip();
	success &= test_qson_deserialize_object_skip();
	return success;
}
